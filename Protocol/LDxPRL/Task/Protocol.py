...
class ProtocolScreen(ProtocolBase):
	...
	def __init__(self, **kwargs):
		...
		self.metadata_cols = [
			...
			, 'show_text_instructions'
			, 'deterministic_probes'
			, 'probabilistic_probes'
			, 'test_location_discrimination'
			, 'include_easy_separation'
			, 'include_hard_separation'
			, 'iti_fixed_or_range'
			, 'iti_length'
			, 'feedback_length'
			, 'block_duration'
			...
			, 'reversal_hit_threshold'
			, 'uncapped_probe_reversals'
			, 'max_reversals'
			, 'training_block_max_reversals'
			, 'image_set'
			]
	...
	def _load_config_parameters(self, parameters_dict):
		...
		self.show_text_instructions = self.parameters_dict.get('show_text_instructions', 'False')

		self.deterministic_probes = self.parameters_dict.get('deterministic_probes', 'True')
		self.probabilistic_probes = self.parameters_dict.get('probabilistic_probes', 'True')

		self.test_location_discrimination = self.parameters_dict.get('test_location_discrimination', 'True')
		self.include_easy_separation = self.parameters_dict.get('include_easy_separation', 'True')
		self.include_hard_separation = self.parameters_dict.get('include_hard_separation', 'True')
		...
		self.probe_block_trial_max = int(self.parameters_dict.get('block_trial_max', '90'))
		
		self.target_reward_probability = float(self.parameters_dict.get('target_reward_probability', '0.8'))
		self.reversal_hit_threshold = int(self.parameters_dict.get('reversal_hit_threshold', '5'))
		self.uncapped_probe_reversals = self.parameters_dict.get('uncapped_probe_reversals', 'True')
		...
		self.training_block_max_reversals = int(self.parameters_dict.get('training_block_max_reversals', '3'))
		...
		if self.deterministic_probes \
				and self.include_hard_separation:
			self.stage_list.append('LD-H')

		if self.deterministic_probes:
			self.stage_list.append('LD-M')

		if self.probabilistic_probes:
			self.stage_list.append('PR-M')

		if self.probabilistic_probes \
				and self.include_easy_separation:
			self.stage_list.append('PR-E')
	...
	def _load_task_variables(self):
		...
		self.block_trial_max = self.probe_block_trial_max
		...
		self.trial_reward_list = []
		self.trial_reward_list_index = -1

	def _setup_session_stages(self):

		if 'PR' in self.current_stage:

			self.nontarget_reward_probability = round(1 - self.target_reward_probability, 2)

			for iTrial in range(int(self.target_reward_probability * self.reversal_hit_threshold)):
				self.trial_reward_list.append('Target')
			
			for iTrial in range(int(self.nontarget_reward_probability * self.reversal_hit_threshold)):
				self.trial_reward_list.append('Nonarget')
			
			self.trial_reward_list = self.constrained_shuffle(self.trial_reward_list, max_run=(self.reversal_hit_threshold - 1))
		
		elif 'LD' in self.current_stage:

			for iTrial in range(self.reversal_hit_threshold - 1):
				self.trial_reward_list.append('Target')
	...
	def _setup_image_widgets(self):
		...
		self.outline_image_path = str(self.image_dir_path / 'whitecircle.png')
		...
		self.text_button_pos_UC = {"center_x": 0.5, "center_y": 0.9}
		...
		# Create stimulus locations

		if self.test_location_discrimination:
			self.stimulus_size = (0.2 * self.width_adjust, 0.2 * self.height_adjust)
			self.stim_x_pos = np.linspace(0.1, 0.9, 6).tolist()

			if 'H' in self.current_stage:
				self.sep = 1
			elif 'E' in self.current_stage:
				self.sep = 3
			else:
				self.sep = 2
			...
			for iLoc in range(len(self.stim_x_pos)):
				self.stim_list.append(ImageButton(source=self.mask_image_path))
				self.stim_list[iLoc].size_hint = self.stimulus_size
				self.stim_list[iLoc].pos_hint = {"center_x": self.stim_x_pos[iLoc], "center_y": 0.5}

		else:
			self.stimulus_size = (0.4 * self.width_adjust, 0.4 * self.height_adjust)
			self.stim_x_pos = [0.25, 0.75]
			self.sep = 1
			self.stim_list = []

			for iLoc in range(len(self.stim_x_pos)):
				self.stim_list.append(ImageButton(source=self.stimulus_image_source))
				self.stim_list[iLoc].size_hint = self.stimulus_size
				self.stim_list[iLoc].pos_hint = {"center_x": self.stim_x_pos[iLoc], "center_y": 0.5}
	...
	def load_parameters(self,parameter_dict):
		...
		# for xCell in self.stim_list:
		# 	self.protocol_floatlayout.add_widget(xCell)
	...
	def stimulus_presentation(self, *args): # Stimulus presentation
		...
		for iLoc in range(len(self.stim_x_pos)):
			...
			if self.test_location_discrimination:
				...
				if (iLoc == (self.sep + 2)) \
						or (iLoc == ((-1 * self.sep) + 3)):
					self.protocol_floatlayout.add_widget(self.stim_list[iLoc])
	...
	def trial_outcomes(self, *args):
		...
		for iLoc in range(len(self.stim_x_pos)):
			...
			if self.test_location_discrimination:
				...
				if (iLoc == (self.sep + 2)) \
						or (iLoc == ((-1 * self.sep) + 3)):
					self.protocol_floatlayout.remove_widget(self.stim_list[iLoc])
	...
	def trial_contingency(self, *args):
		...
		try:
			if self.current_block_trial != 0:
				...
				# 	and (sum(self.current_reversal) >= self.training_block_max_reversals):
			...
			if self.current_hits >= self.reversal_hit_threshold:
				self.current_hits = 0
				self.current_reversal += 1

				if (self.current_stage == 'Training') \
						and (self.current_reversal >= self.training_block_max_reversals):
					self.stage_results_screen()

				elif not self.uncapped_probe_reversals \
						and (self.current_reversal > self.max_reversals):
					self.stage_results_screen()
	...
	def results_screen(self, *args):
		...
		self.result_str = 'Great job! You have earned ' + str(self.point_counter) + ' points!'
	...
	def stage_results_screen(self, *args):
		...
		if self.stage_index < (len(self.stage_list) - 1) \
			...
			Clock.schedule_once(self.stage_results_screen_end, 1.0)
		...
		if self.current_stage == 'Training':
			self.result_label_str = 'Great job! You have completed the training block.' \
					+ '\n\n' \
					+ self.stage_string
			
		else:
			self.result_label_str = 'Great job! You have earned ' \
	...
	def stage_results_screen_end(self, *args):
	...
	def block_contingency(self, *args):
		...
		try:
			...
			if self.current_stage == 'Training':
				self.sep = 2
			elif 'H' in self.current_stage:
				self.sep = 1
			elif 'M' in self.current_stage:
				self.sep = 2
			elif 'E' in self.current_stage:
				self.sep = 3
			...
			for iLoc in range(len(self.stim_x_pos)):
				...
				if self.test_location_discrimination:
					...
					if (iLoc == (self.sep + 2)) \
							or (iLoc == ((-1 * self.sep) + 3)):
						self.stim_list[iLoc].texture = self.image_dict[self.target_image].image.texture
						
						if iLoc == (self.sep + 2):
							self.stim_list[iLoc].bind(on_press=self.right_pressed)
						elif iLoc == ((-1 * self.sep) + 3):
							self.stim_list[iLoc].bind(on_press=self.left_pressed)

					else:
						self.stim_list[iLoc].texture = self.image_dict[self.mask_image].image.texture
						self.stim_list[iLoc].unbind(on_press=self.right_pressed)
						self.stim_list[iLoc].unbind(on_press=self.left_pressed)
				
				else:
					self.stim_list[0].bind(on_press=self.left_pressed)
					self.stim_list[1].bind(on_press=self.right_pressed)
			...
			if self.show_text_instructions \
					and (self.current_block <= 1):
